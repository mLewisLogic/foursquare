...
__version__ = "1!{year}.{month}.{day}".format(
    year=API_VERSION_YEAR, month=API_VERSION_MONTH, day=API_VERSION_DAY
)
...
class Foursquare(object):
    ...
    class OAuth(object):
        ...
        def auth_url(self):
            ...
            return "{AUTH_ENDPOINT}?{params}".format(
                AUTH_ENDPOINT=AUTH_ENDPOINT, params=parse.urlencode(params)
            )
    ...
    class Requester(object):
        ...
        def GET(self, path, params={}, **kwargs):
            ...
            url = "{API_ENDPOINT}{path}".format(API_ENDPOINT=API_ENDPOINT, path=path)
        ...
        def POST(self, path, data={}, files=None):
            ...
            url = "{API_ENDPOINT}{path}".format(API_ENDPOINT=API_ENDPOINT, path=path)
            result = _post(
                url, headers=headers, data=data, files=files, timeout=self.post_timeout
            )
            self.rate_limit = result["headers"]["X-RateLimit-Limit"]
            self.rate_remaining = result["headers"]["X-RateLimit-Remaining"]
            return result["data"]["response"]
    ...
    class Users(_Endpoint):
        ...
        def checkins(self, USER_ID=u"self", params={}, multi=False):
            ...
            return self.GET(
                "{USER_ID}/checkins".format(USER_ID=USER_ID), params, multi=multi
            )
        ...
        def all_checkins(self, USER_ID=u"self"):
            ...
            while True:
                ...
                if (offset >= checkins["checkins"]["count"]) or (
                    len(checkins["checkins"]["items"]) == 0
                ):
    ...
    class Checkins(_Endpoint):
        ...
        def __call__(self, CHECKIN_ID, params={}, multi=False):
            ...
            return self.GET(
                "{CHECKIN_ID}".format(CHECKIN_ID=CHECKIN_ID), params, multi=multi
            )
    ...
    class Tips(_Endpoint):
        ...
        def listed(self, TIP_ID, params={}, multi=False):
            ...
            return self.GET(
                "{TIP_ID}/listed".format(TIP_ID=TIP_ID), params, multi=multi
            )
    ...
    class Specials(_Endpoint):
        ...
        def __call__(self, SPECIAL_ID, params, multi=False):
            ...
            return self.GET(
                "{SPECIAL_ID}".format(SPECIAL_ID=SPECIAL_ID), params, multi=multi
            )
    ...
    class Pages(_Endpoint):
        ...
        def venues(self, PAGE_ID, params={}, multi=False):
            ...
            return self.GET(
                "{PAGE_ID}/venues".format(PAGE_ID=PAGE_ID), params, multi=multi
            )
