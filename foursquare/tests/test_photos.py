...
class PhotosEndpointTestCase(BaseAuthenticatedEndpointTestCase):
    ...
    def test_attach_photo(self):
        ...
        if os.path.isfile(test_photo):
            ...
            try:
                response = self.api.photos.add(
                    params={"checkinId": checkin["id"]}, photo_data=photo_data
                )
                assert "photo" in response
            ...
            print(
                u"Put a 'test-photo.jpg' file in the testdata/ directory to enable this test."
            )
