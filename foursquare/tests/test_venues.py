...
class VenuesEndpointTestCase(BaseAuthenticatedEndpointTestCase):
    ...
    def test_explore_radius(self):
        response = self.api.venues.explore(
            params={"ll": self.default_geo, "radius": 30}
        )
        assert "groups" in response

    def test_explore_section(self):
        response = self.api.venues.explore(
            params={"ll": self.default_geo, "section": u"café"}
        )
        assert "groups" in response

    def test_explore_query(self):
        response = self.api.venues.explore(
            params={"ll": self.default_geo, "query": "donuts"}
        )
        assert "groups" in response
    ...
    def test_explore_intent(self):
        response = self.api.venues.explore(
            params={"ll": self.default_geo, "intent": "specials"}
        )
        assert "groups" in response
    ...
    def test_search_query(self):
        response = self.api.venues.search(
            params={"ll": self.default_geo, "query": "donuts"}
        )
        assert "venues" in response
    ...
    def test_search_browse(self):
        response = self.api.venues.search(
            params={
                "ll": self.default_geo,
                "radius": self.default_geo_radius,
                "intent": "browse",
            }
        )
        assert "venues" in response
    ...
    def test_suggestcompletion(self):
        response = self.api.venues.suggestcompletion(
            params={"ll": self.default_geo, "query": "cof"}
        )
    ...
    def test_trending_limit(self):
        response = self.api.venues.trending(
            params={"ll": self.default_geo, "limit": 10}
        )
        assert "venues" in response

    def test_trending_radius(self):
        response = self.api.venues.trending(
            params={"ll": self.default_geo, "radius": 100}
        )
        assert "venues" in response
    ...
    def test_listed_group(self):
        response = self.api.venues.listed(
            self.default_venueid, params={"group": "friends"}
        )
        assert "lists" in response
    ...
    def test_photos(self):
        response = self.api.venues.photos(
            self.default_venueid, params={"group": "venue"}
        )
    ...
    def test_tips_group(self):
        response = self.api.venues.tips(
            self.default_venueid, params={"sort": "popular"}
        )
        assert "tips" in response
...
class VenuesUserlessEndpointTestCase(BaseUserlessEndpointTestCase):
    ...
    def test_explore_section(self):
        response = self.api.venues.explore(
            params={"ll": self.default_geo, "section": "coffee"}
        )
        assert "groups" in response
    ...
    def test_search_ampersand(self):
        response = self.api.venues.search(
            params={"query": u"Mirch Masala Restaurant & Bar", "ll": u"22.52,88.36"}
        )
        assert "venues" in response
    ...
    def test_listed_group(self):
        response = self.api.venues.listed(
            self.default_venueid, params={"group": "other"}
        )
        assert "lists" in response
