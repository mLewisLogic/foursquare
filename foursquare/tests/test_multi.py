...
class MultiEndpointTestCase(BaseAuthenticatedEndpointTestCase):
    ...
    def test_multi(self):
        ...
        self.api.pages.venues(
            self.default_pageid, params={"limit": 10, "offset": 10}, multi=True
        )
        ...
        assert len(self.api.multi) == len(
            expected_responses
        ), u"{0} requests queued. Expecting {1}".format(
            len(self.api.multi), len(expected_responses)
        )
        assert (
            self.api.multi.num_required_api_calls == 2
        ), u"{0} required API calls. Expecting 2".format(
        ...
        for response, expected_response in zip(self.api.multi(), expected_responses):
            assert expected_response in response, "{0} not in response".format(
                expected_response
            )
